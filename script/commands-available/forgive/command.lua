...
local function text_callback(cn, text)
  if (string.match(text:lower(), "^np") or string.match(text:lower(), "no problem")) then
    server.player_msg(cn, "forgive_analysetext_message")
  end
end
...
local function run(cn)
  ...
  for _,actor_teamkilled in pairs(teamkills[actor_id]["teamkilled"]) do
    if cn_id == actor_teamkilled then
      ...
      server.player_msg(actor_cn, "forgive_actor_forgiven_message", { playername = server.player_displayname(cn) })
      server.player_msg(cn, "forgive_target_forgiven_message", {playername=server.player_displayname(actor_cn) })
      return
    end
  end
