...
local queue = {}
local igamemode = ""
local mysql_settings = {
        hostname    = "127.0.0.1",
        port        = "3306",
        username    = "tbmcpartyauth",
        password    = "8XA9dG5TN66sKC3b",
        database    = "tbmcpartyauthclan",
        install     = false,
        servername  = "classic rugby party 2"
      }
...
local function install_db(settings)
...
local function mysql_open(settings)
  if not dbcon then
    dbcon = luasql.mysql():connect(settings.database, settings.username, settings.password, settings.hostname, settings.port)
...
local function execute_statement(statement)
    if not dbcon and not mysql_open(mysql_settings) then
       return false
    end
    local cursor, errorinfo = dbcon:execute(statement)
    if not cursor then
...
local function mysql_commit_flagrun(flagrun)
    ...
    if not dbcon and not mysql_open(open_settings) then
       return false
    end
